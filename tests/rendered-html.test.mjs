...
test("ships PWA and custom UK map configuration", async () => {
  ...
  assert.match(mapEngine, /const APP_VERSION = "v2\.3\.0"/);
  assert.match(serviceWorker, /map-engine-shell-v1168/);
  ...
  assert.match(settingsPanel, /Route calculation/);
  assert.match(settingsPanel, /Fast/);
  assert.match(settingsPanel, /route-profile-setting/);
  assert.match(settingsPanel, /aria-label="Route calculation preference"/);
  assert.match(settingsPanel, /Avoid narrow lanes/);
  assert.match(settingsPanel, /Fast optimises drive time/);
