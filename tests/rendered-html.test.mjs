...
test("ships PWA and custom UK map configuration", async () => {
  ...
  assert.match(mapEngine, /const APP_VERSION = "v2\.10\.83"/);
  assert.match(serviceWorker, /map-engine-shell-v1265/);
  ...
  assert.match(mapEngine, /\[\"place-village\", \["interpolate", \["linear"\], \["zoom"\], 12, 9, 14, 11, 16, 12\.5\]\]/);
  assert.match(mapEngine, /for \(const \[layer, size\] of PLACE_LAYOUTS\)/);
  assert.match(mapEngine, /PLACE_MIN_ZOOM/);
  assert.equal(mapStyle.layers.find((layer) => layer.id === "place-city").minzoom, 4);
  assert.equal(mapStyle.layers.find((layer) => layer.id === "place-town").minzoom, 8);
  assert.equal(mapStyle.layers.find((layer) => layer.id === "place-village").minzoom, 12);
  const centreFeatures = mapStyle.sources["uk-postcode-centres"].data.features;
  const centreDistricts = centreFeatures.map((feature) => feature.properties.district);
  assert.ok(centreDistricts.includes("B1"));
  assert.ok(centreDistricts.includes("WV1"));
  assert.ok(centreDistricts.includes("CV1"));
  assert.match(mapEngine, /stickyLandmarksAhead/);
  assert.ok(mapStyle.layers.find((layer) => layer.id === "place-centre"));
  assert.ok(mapStyle.layers.find((layer) => layer.id === "place-centre-sub"));
  assert.equal(mapStyle.layers.find((layer) => layer.id === "place-centre").minzoom, 5);
  assert.equal(mapStyle.layers.find((layer) => layer.id === "place-centre-sub").minzoom, 7);
  assert.ok(mapStyle.layers.find((layer) => layer.id === "place-centre").filter[2] === 1);
