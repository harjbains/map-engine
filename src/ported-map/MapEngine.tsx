...
import { V3UberOverlay } from "../map/V3UberOverlay";
import type { UberDashboard, WeeklySummary, WorkWeight } from "../uber/types";
import type { EarningsTransition } from "../uber/milestones";
...
import { styleJsonUrl } from "./lib/mapbox-client";
...
export default function MapEngine({ dashboard, preview, onSaveTodayEarnings, onSaveMileage, onSavePlan, onLoadHistory, onChangeDate, onSignOut, onStartSession, onPauseSession, onResumeSession, onEndSession, transition }: {
  ...
  const landmarks = useLandmarks({ fix, roadName: currentRoad, mapReady, enabled: settings.showLandmarks, online, route: activeRoute });
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: styleJsonUrl(),
      transformRequest: (url) => ({ url: mapResourceUrl(url, window.location.origin, import.meta.env.DEV) }),
    ...
    map.on("error", (event) => {
      ...
      else if (event.error) setMapMessage(`Map resource failed: ${event.error.message}`);
    });
  ...
  return (
    <main className={`drive-shell ${settings.darkMode ? "classic-dark dark" : "classic"}${dashboard ? " uber-active" : ""}`}>
