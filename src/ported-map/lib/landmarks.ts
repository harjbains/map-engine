...
way(around:${Math.round(radius)},${centre.latitude.toFixed(6)},${centre.longitude.toFixed(6)})["landuse"~"^(retail|commercial)$"]["name"];
  ...
  const roadLines: Array<{ name: string | null; geom: Array<{ lat: number; lon: number }> }> = [];
  for (const element of roadPayload?.elements ?? []) {
    ...
    if (typeof tags.highway === "string" && DRIVABLE_HIGHWAYS.has(tags.highway) && Array.isArray(element.geometry)) {
      roadLines.push({ name: tags.name ?? null, geom: element.geometry as Array<{ lat: number; lon: number }> });
    }
  }
  ...
  for (const element of poiPayload?.elements ?? []) {
    ...
    if (!point) continue;
    const roadMatch = roadLines.length ? nearestMainRoadMetres(point, roadLines) : { metres: 0, name: null };
    if (roadMatch.metres > ROAD_MAX_METRES) continue;
    landmarks.push({
      ...
      mainroadMetres: roadMatch.metres,
