...
export function useLandmarks({ fix, roadName, mapReady, enabled, online, route }: UseLandmarksOptions) {
  ...
  useEffect(() => {
    const next = enabled && fix ? stickyLandmarksAhead(fix, landmarks, previousVisibleRef.current, LANDMARK_CHIP_LIMIT, route, roadName) : [];
    previousVisibleRef.current = next;
    setVisible(next);
  }, [fix, landmarks, enabled, route, roadName]);
