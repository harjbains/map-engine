  ...
  return <>
        ...
        <button type="button" className={`session-progress session-progress-${forecastBand}`} onClick={() => setModal("editor")} aria-label="Open Update Earnings screen">
          ...
          <div className="session-progress-track">
            <div className="session-progress-fill" style={{ width: `${percent}%` }}>
              <span className="session-progress-text">{percent}%</span>
            </div>
          </div>
        </button>
    ...
    {modal !== "closed" && <div className="uber-modal-backdrop" role="presentation" onMouseDown={() => setModal("closed")}>
      <section className="uber-modal" role="dialog" aria-modal="true" aria-label="Uber earnings dashboard" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {modal === "editor" && <DailyEarningsPanel key={dashboard.today} dashboard={dashboard} onCancel={() => setModal(returnTo === "history" ? "history" : "dashboard")} onSave={async (previewPence) => { await onSaveTodayEarnings(previewPence); setModal("closed"); if (returnTo === "closed") onChangeDate?.(null); }} />}
        {modal === "mileage" && <MileagePanel key={dashboard.today} dashboard={dashboard} onCancel={() => setModal(returnTo === "history" ? "history" : "dashboard")} onSave={async (miles) => { await onSaveMileage(miles); setModal("closed"); if (returnTo === "closed") onChangeDate?.(null); }} />}
